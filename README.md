...
Result mapping: an active foreign lease is `NO-OP`; an internal implementable deficiency or unknown failure enters `AUTONOMOUS_RECOVERY_LOOP`; preserved incomplete work is `PARTIAL`; only a proven `EXTERNAL_BLOCKER` or irrecoverable coordination condition after the full ladder is `BLOCKED`. Every recovery must be followed by a fresh remote-state read before resuming.
