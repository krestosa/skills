...
def bundle(path,routes=None,profiles=None,files=None,exclude_files=None):
 items=[]
 ...
 items=[x for x in items if not (x[0]=="file" and x[1] in exclude_files)]
...
bundle(out/"19-CONTRACTS.md",files=["contracts/authorization-envelope.schema.json","contracts/connector-contracts.md"])
bundle(out/"20-PROMPTING-GPT56.md",routes=["prompt-authoring"])
(out/"21-PROJECT-INSTRUCTIONS.txt").write_text("Use SKILL.md as the entrypoint. Apply the GPT-5.6 Sol outcome contract, load the smallest route and only the detected stack profile, keep updates sparse, stop when success criteria are met, and use the GitHub connector for every remote GitHub operation. Local git is local-only.\n")
